...
select_port_test() {
    local port_test
    read -p "Введите порт для ТЕСТA! Byedpi (по умолчанию 20000): " port_test
    port_test=${port_test:-20000}
...
EOF
        ...
        rm -f "$HOME/.config/systemd/user/ciadpitest.service"
        rm -f "$HOME/.config/byedpitest.conf"
...
EOF
    ...
    echo
    log yellow "1 - Установка с тестом (автоподбор стратегии)"
    log yellow "2 - Установка без теста (ручной ввод стратегии)"
    log yellow "3 - Только тестирование конфигурации"
    log yellow "4 - Поменять порт у службы"
    read selector
    ...
    case "$selector" in
        ...
        "2")
            port=$(select_port)
            echo
            read -p "Вставьте вашу стратегию (аргументы для ciadpi): " custom_strategy
            if [[ -z "$custom_strategy" ]]; then
                log red "Ошибка: Стратегия не может быть пустой."
                exit 1
            fi
            ;;
        "3")
    ...
    if [[ "$selector" == "2" ]]; then
        update_service "$port" "$custom_strategy"
        log green "==========================================================="
        log green "Установка ByeDPI завершена. Служба ciadpi запущена!"
        log green "Ваша стратегия: $custom_strategy"
